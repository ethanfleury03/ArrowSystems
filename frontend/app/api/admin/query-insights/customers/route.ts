...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await iamBackendGet('/admin/query-insights/customers', {
      'X-User-Token': token,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { detail: error.detail || 'Failed to fetch customers' },
    ...
    console.error('Admin query insights customers API error:', error);
    return NextResponse.json(
